...
class KerasTextClassifier(BaseTextClassifier):
    def __init__(self, tokenizer, word2vec, model_path, max_length=20, n_epochs=50, batch_size=6,
...
def test():
    ...
    keras_text_classifier = BiDirectionalLSTMClassifier(tokenizer=tokenizer, word2vec=word2vec_model.wv,
                                                        model_path=dir_path + '/../models/sentiment_model_4.h5',
                                                        max_length=200, n_epochs=50,
                                                        sym_dict=sym_dict)
    X, y = keras_text_classifier.load_data([dir_path + '/../data/sentiment/training_data/positive.txt',
                                            dir_path + '/../data/sentiment/training_data/negative.txt',
                                            dir_path + '/../data/sentiment/training_data/neutral.txt'],
    ...
    #test_sentences = ['em cũng đang dùng mà mãi không hư']
    #labels = keras_text_classifier.classify(test_sentences, label_dict=label_dict)
    #print(labels)  # Output: ['tiêu cực', 'tích cực', ]
